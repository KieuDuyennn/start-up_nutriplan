...
```mermaid
%%{init: {'theme':'base','themeVariables':{'fontFamily':'Segoe UI, Roboto, sans-serif','fontSize':'13px','lineColor':'#90A4AE'}}}%%
flowchart TD
  V(["Vốn phần marketing"])
  V --> A["1. Cỗ máy thu hút<br/>lặp lại được"]
  V --> B["2. Quảng cáo trả phí<br/>có kiểm soát"]
  V --> C["3. Micro KOC<br/>sinh viên"]
  V --> D["4. Nội dung và<br/>tài sản thương hiệu"]
  V --> E["5. Giới thiệu và<br/>ưu đãi ra mắt"]
  V --> F["6. Sự kiện và demo<br/>tại cụm trường"]
  V --> G["7. Công cụ đo lường<br/>và tăng trưởng"]
  V --> H["8. Đội ngũ nội dung<br/>và growth"]
  A --> K{{"Mục tiêu chung:<br/>CAC giảm, khách trả phí tăng,<br/>thương hiệu dẫn đầu phân khúc SV"}}
  B --> K
  C --> K
  D --> K
  E --> K
  ...
  classDef goal fill:#FFF8E1,stroke:#FF8F00,stroke-width:2px,color:#E65100;
  class V root;
  class A,B,C,D,E,F,G,H leaf;
  class K goal;
```
